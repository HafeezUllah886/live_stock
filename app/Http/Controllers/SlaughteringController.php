...
class SlaughteringController extends Controller
{
    ...
    public function create()
    {
        ...
        $factories = accounts::customerAndFactory()->get();
    ...
    {
        $slaughter = Slaughtering::find($id);
        $products = products::active()->get();
        $customers = accounts::customer()->get();
        $factories = accounts::factory()->get();
        $butchers = accounts::butcher()->get();

        return view('slaughtering.edit', compact('slaughter', 'products', 'customers', 'factories', 'butchers'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        createStock($request->product_id, 0, $request->qty, $request->date, $notes_for_stock, $ref);
        return redirect()->route('slaughter.index')->with('success', 'Slaughtering updated successfully');
        }
        catch(Exception $e)
        {
            return redirect()->route('slaughter.index')->with('error', 'Slaughtering updated failed');
        }
    }
    ...
    public function destroy($id)
    {
        try
        {
            $slaughtering = Slaughtering::find($id);
            $slaughtering->delete();
            return redirect()->route('slaughter.index')->with('success', 'Slaughtering deleted successfully');
        }
        catch(Exception $e)
        {
            return redirect()->route('slaughter.index')->with('error', 'Slaughtering deleted failed');
        }
    }
}
