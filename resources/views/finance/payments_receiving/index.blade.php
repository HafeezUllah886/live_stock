                                               ...
                                               {{--  <li>
                                                ...
                                                <li>
                                                    <a class="dropdown-item text-danger"
                                                        href="{{ route('payments.delete', $tran->refID) }}">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group mt-2">
                                    <label for="depositer_id">Reposited By (Balance: <span id="accountBalanceDepositer">0</span>)</label>
                                    <select name="depositer_id" id="depositer_id" onchange="getBalanceDepositer()" required
                                        class="selectize">
                                        <option value=""></option>
                                        @foreach ($depositers as $depositor)
                                            <option value="{{ $depositor->id }}">{{ $depositor->title }}
                                                ({{ $depositor->category }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group mt-2">
                                    <label for="account_id">Received In (Balance: <span id="accountBalance">0</span>)</label>
...
@section('page-js')
    ...
    <script>
        ...
        function getBalanceDepositer() {
            var id = $("#depositer_id").find(":selected").val();
            $.ajax({
                url: "{{ url('/accountbalance/') }}/" + id,
                method: 'GET',
                success: function(response) {
                    console.log(response);
                    $("#accountBalanceDepositer").html(response.data.toFixed(0));
                    if (response.data > 0) {
                        $("#accountBalanceDepositer").addClass('text-success');
                        $("#accountBalanceDepositer").removeClass('text-danger');
                    } else {
                        $("#accountBalanceDepositer").addClass('text-danger');
                        $("#accountBalanceDepositer").removeClass('text-success');
                    }
                },
